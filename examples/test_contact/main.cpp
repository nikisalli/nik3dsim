...
int main() {
    ...
    add_rigidbody(&m, BODY_SPHERE, (float[]){0.4f, 1.0f, 1.0f}, 1.0f, (float[]){2, 0, 10.0f}, (float[]){0, 0, 0}, 0.001f, 0.0f, 1, 1);
    add_rigidbody(&m, BODY_SPHERE, (float[]){0.4f, 1.0f, 1.0f}, 1.0f, (float[]){1, 0.1, 10.0f}, (float[]){0, 0, 0}, 0.001f, 0.0f, 1, 1);
    
    add_staticbody(&m, BODY_PLANE, (float[]){0.0f, 0.0f, 0.0f}, (float[]){2, 0, 0}, (float[]){0, -M_PI * 0.4, 0}, 0.001f, 0.0f, 1, 1);
    add_staticbody(&m, BODY_PLANE, (float[]){0.0f, 0.0f, 0.0f}, (float[]){-2, 0, 0}, (float[]){0, M_PI * 0.4, 0}, 0.001f, 0.0f, 1, 1);

    float local_pos[3] = {0, 0, 0};
    add_distance_constraint(&m, 0, 1, local_pos, local_pos, 0.0f, 1.0f);
