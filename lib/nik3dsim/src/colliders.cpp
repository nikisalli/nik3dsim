...
namespace nik3dsim {
    ...
    Contact collide_capsule_box(const niknum cpos[3], const niknum crot[4], const niknum csize[3], 
                            const niknum bpos[3], const niknum brot[4], const niknum ibrot[4], const niknum bsize[3]) {
        Contact contact = {0};
        
        // Transform capsule endpoints into box space
        niknum capsuleDir[3] = {0, 0, 1};  // Default capsule direction
        vec3_quat_rotate(capsuleDir, crot, capsuleDir);  // Rotate to world space
        
        niknum segStart[3], segEnd[3], relStart[3], relEnd[3];
        // Calculate segment endpoints in world space
        vec3_addscl(segStart, cpos, capsuleDir, csize[1]/2);
        vec3_addscl(segEnd, cpos, capsuleDir, -csize[1]/2);
        
        // Transform endpoints to box space
        niknum temp[3];
        ...
        // Find closest points using segment_box_query
        niknum query_result[4];
        segment_box_query(query_result, relStart, relEnd, bsize);
        
        // Calculate the actual point on the segment
        niknum d[3];
        ...
        niknum worldBoxPoint[3], worldSegPoint[3];
        vec3_quat_rotate(temp, brot, query_result);
        vec3_add(worldBoxPoint, temp, bpos);
        
        vec3_quat_rotate(temp, brot, segmentPoint);
        vec3_add(worldSegPoint, temp, bpos);
        
        // Calculate contact normal and depth
        niknum normal[3];
        vec3_sub(normal, worldSegPoint, worldBoxPoint);
        niknum dist = vec3_normalize(normal, normal);
        
        // Set contact information
        vec3_scl(contact.n, normal, -1.0);
        vec3_copy(contact.pos, worldBoxPoint);
